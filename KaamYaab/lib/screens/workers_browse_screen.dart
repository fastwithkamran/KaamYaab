...
class _WorkersBrowseScreenState extends State<WorkersBrowseScreen> {
  ...
  bool _isCategoryMatch(String? workerCategory, String selectedCategory) {
    if (workerCategory == null) return false;
    if (selectedCategory == 'All') return true;

    final w = workerCategory.trim().toLowerCase();
    final s = selectedCategory.trim().toLowerCase();
    if (w == s) return true;

    // Normalizations
    if (s == 'plumber' && w.contains('plumb')) return true;
    if (s == 'electrician' && (w.contains('electric') || w.contains('solar'))) return true;
    if (s == 'ac technician' && (w.contains('ac') || w.contains('air') || w.contains('cooling'))) return true;
    if (s == 'carpenter' && (w.contains('carpent') || w.contains('wood'))) return true;
    if (s == 'painter' && w.contains('paint')) return true;
    if (s == 'cleaner' && w.contains('clean')) return true;
    if (s == 'driver' && w.contains('driv')) return true;
    if (s == 'security guard' && (w.contains('security') || w.contains('guard'))) return true;
    if (s == 'cook' && (w.contains('cook') || w.contains('chef') || w.contains('food'))) return true;
    if (s == 'mason' && w.contains('mason')) return true;

    // Reverse check
    if (w == 'plumber' && s.contains('plumb')) return true;
    if (w == 'electrician' && (s.contains('electric') || s.contains('solar'))) return true;
    if (w == 'ac technician' && (s.contains('ac') || s.contains('air') || s.contains('cooling'))) return true;
    if (w == 'carpenter' && (s.contains('carpent') || s.contains('wood'))) return true;
    if (w == 'painter' && s.contains('paint')) return true;
    if (w == 'cleaner' && s.contains('clean')) return true;
    if (w == 'driver' && s.contains('driv')) return true;
    if (w == 'security guard' && (s.contains('security') || s.contains('guard'))) return true;
    if (w == 'cook' && (s.contains('cook') || s.contains('chef') || s.contains('food'))) return true;
    if (w == 'mason' && s.contains('mason')) return true;

    return w.contains(s) || s.contains(w);
  }
  ...
  void _applyFilters() {
    ...
    if (_selectedCategory != null && _selectedCategory != 'All') {
      result =
          result.where((w) => _isCategoryMatch(w.serviceCategory, _selectedCategory!)).toList();
    }
    ...
    switch (_sortBy) {
      ...
      case 'price_low':
        result.sort(
          (a, b) => (a.baseRatePkr ?? 0).compareTo(b.baseRatePkr ?? 0),
        );
      case 'price_high':
        result.sort(
          (a, b) => (b.baseRatePkr ?? 0).compareTo(a.baseRatePkr ?? 0),
        );
      case 'jobs':
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              // In _buildHeader(), replace the IconButton with:
  ...
  Widget _buildSortAndFilter() {
    return Padding(
      ...
      child: Row(
        children: [
          Text(
            '${_filtered.length} workers found',
...
class _WorkerCard extends StatelessWidget {
                      ...
                      Positioned(
                        ...
                        child: Container(
                          ...
                          decoration: BoxDecoration(
                            color:
                                worker.isAvailable
                                    ? AppTheme.greenSuccess.withValues(
                                      alpha: 0.9,
                                    )
                                    : Colors.black54,
                  ...
                  child: Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(
                          worker.subRole ?? worker.serviceCategory ?? '',
                          style: const TextStyle(
                            color: AppTheme.tealPrimary,
                            fontSize: 11,
  ...
  Widget _buildProfileImage() {
    ...
    final colors =
        _avatarColors[worker.serviceCategory] ??
        [AppTheme.tealDark, AppTheme.tealPrimary];
    return Container(
      height: 130,
      width: double.infinity,
      decoration: BoxDecoration(
      ...
      child: Center(
        child: Column(
          ...
          children: [
            Text(
              _categoryEmoji[worker.serviceCategory] ?? '👷',
              style: const TextStyle(fontSize: 38),
            ),
            const SizedBox(height: 4),
            Text(
              worker.name.split(' ').where((e) => e.isNotEmpty).map((e) => e[0]).take(2).join(),
              style: const TextStyle(
