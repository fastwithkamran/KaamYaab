...
class AiService {
  ...
  static final String _geminiKey = RuntimeConfig.geminiApiKey.trim();

  static bool get _hasCohereKey => _cohereKey.isNotEmpty && _cohereKey != 'YOUR_COHERE_API_KEY_HERE';
  static bool get _hasGeminiKey => _geminiKey.isNotEmpty && _geminiKey != 'YOUR_GEMINI_API_KEY_HERE';
  ...
  static Future<Map<String, dynamic>> chat({
    ...
    required List<Map<String, dynamic>> geminiHistory,
...
''';
    ...
    if (_hasGeminiKey) {
      final result = await _callGemini(userMessage, systemPrompt: systemPrompt, history: geminiHistory);
...
''';
    ...
    if (result == null && _hasGeminiKey) result = await _callGemini(prompt, systemPrompt: preamble);
...
''';
  ...
  // ─── Gemini API Helper ───────────────────────────────────────────────────────
  static Future<dynamic> _callGemini(String prompt, {String? systemPrompt, List<Map<String, dynamic>>? history}) async {
    try {
      final url = '$_geminiUrl?key=$_geminiKey';
      final contents = history != null ? List<Map<String, dynamic>>.from(history) : <Map<String, dynamic>>[];
      contents.add({
        'role': 'user',
        'parts': [{'text': prompt}]
      });
      ...
      final payload = {
        'contents': contents,
        'systemInstruction': systemPrompt != null ? {'parts': [{'text': systemPrompt}]} : null,
  ...
  static Future<Map<String, dynamic>> extractIntent(String rawInput) async {
    final res = await chat(userMessage: rawInput, geminiHistory: [], cohereHistory: [], userArea: 'Islamabad', userLanguage: 'english');
  ...
  static List<Map<String, dynamic>> _convertToGeminiHistory(List<Map<String, String>> history) {
...
''';
    ...
    if (result == null && _hasGeminiKey) {
      result = await _callGemini(prompt, systemPrompt: systemPrompt);
    }
...
''';
    ...
    if (result == null && _hasGeminiKey) {
      final geminiHistory = _convertToGeminiHistory(chatHistory);
      result = await _callGemini(message, systemPrompt: systemPrompt, history: geminiHistory);
    }
