...
class MatchingService {
    ...
    try {
      ...
      for (var worker in liveWorkers) {
        ...
        if (worker.latitude != null && worker.longitude != null) {
          // Derive experience level from years instead of hardcoding
          final expLevel = _deriveExperienceLevel(worker.experienceYears);
          // Derive conservative defaults for new workers based on actual data
          final isNewWorker = worker.totalJobs < 5;
          final effectiveRating = worker.rating > 0 ? worker.rating : (isNewWorker ? 3.5 : 4.0);
          // Compute a basic DNA score from available data instead of flat 800
          final computedDna = _computeNewWorkerDna(
            rating: effectiveRating,
            totalJobs: worker.totalJobs,
            experienceYears: worker.experienceYears,
          );
          // Parse available slots from availability rules instead of hardcoding
  ...
  static double _calculateQuote(
    ...
    final distanceCharge = dist > 3 ? (dist - 3) * 20 : 0.0; // Higher distance cost
